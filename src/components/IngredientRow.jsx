    ...
    return (
      <Motion.article layout id={`ingredient-${item.id}`} className="rounded-2xl border border-stone-200/80 bg-white/90 p-3 shadow-[0_14px_34px_rgba(65,48,25,0.06)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-base font-semibold text-stone-800">{displayName}</p>
            <p className="mt-1 text-xs text-stone-500">{t(language, 'ingredient.addLinesDescription')}</p>
          </div>
              ...
              <Fragment key={line.id}>
                <div className="rounded-2xl border border-stone-200 bg-stone-50/80 p-3">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-stone-500">{t(language, 'ingredient.line', { count: index + 1 })}</p>
                    {entry.entries.length > 1 ? (
                      <button type="button" onClick={() => onRemoveEntry(item.id, line.id)} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-500 transition hover:border-red-200 hover:text-red-600" aria-label={t(language, 'ingredient.removeLine', { item: item.name, count: index + 1 })}>
                        <IoTrashOutline />
                      </button>
                    ) : null}
                  </div>
                  <div className={`grid gap-3 sm:items-center ${hasOptions ? 'sm:grid-cols-[120px_150px_110px]' : 'sm:grid-cols-[120px_110px]'}`}>
                    <label className="block">
                      <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.18em] text-stone-500">{t(language, 'ingredient.quantity')}</span>
                    ...
                    {hasOptions ? (
                      <label className="block">
                        ...
                        <select value={line.option} onChange={(event) => onEntryChange(item.id, line.id, 'option', event.target.value)} onFocus={() => setFocusedField(`option-${line.id}`)} onBlur={() => setFocusedField('')} className="h-12 w-full rounded-xl border border-stone-200 bg-white px-3 text-base text-stone-800 outline-none transition focus:border-amber-400">
                          <option value="">{t(language, 'ingredient.select')}</option>
                    ...
                    <label className="block">
                      <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.18em] text-stone-500">{t(language, 'ingredient.unit')}</span>
                  ...
                  {hasOptions && isCustomOption ? (
                    <label className="mt-3 block">
                      <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.18em] text-stone-500">{t(language, 'ingredient.customSize')}</span>
  ...
  return (
    <Motion.article layout id={`ingredient-${item.id}`} animate={{ scale: focusedField ? 1.01 : 1, y: focusedField ? -1 : 0 }} transition={{ duration: 0.16, ease: 'easeOut' }} className="rounded-2xl border border-stone-200/80 bg-white/90 p-3 shadow-[0_14px_34px_rgba(65,48,25,0.06)]">
      <div className={`grid gap-3 sm:items-center ${hasOptions ? 'sm:grid-cols-[minmax(0,1fr)_120px_110px_150px]' : 'sm:grid-cols-[minmax(0,1fr)_110px_96px]'}`}>
        <div>
          <p className="text-base font-semibold text-stone-800">{displayName}</p>
          <p className="mt-1 text-xs text-stone-500">{hasOptions ? t(language, 'ingredient.variantHint') : t(language, 'ingredient.predefined')}</p>
        </div>
        <label className="block">
          <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.18em] text-stone-500">{t(language, 'ingredient.quantity')}</span>
